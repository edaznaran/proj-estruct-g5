...
void productosxdefecto(TpProducto &lista,string nombre,int codigo,float precio,int stock,string categoria){	//Agrega un producto por codigo
...
void insertarProductosxdefecto(TpProducto &productos){				//Lista est�tica de productos base
...
TpCliente validarCliente(TpCliente lista){		//Busca y devuelve un usuario de la lista
	TpCliente q=NULL;
	q=new (struct cliente);
	string usuario;
	cout<<endl<<"\tUsuario: ",cin>>usuario;
	while(lista!=NULL){
...
void vectorCategorias(TpProducto lista,vector<string>&categorias){	//Crea un vector con las categorias de la estructura lista
	int cond=0;
	while(lista!=NULL){
...
string tab(string cadena){	//Ayuda para ordenar el catalogo
	if(cadena.size()>7){
		return "\t";
	}
	else{
...
float mostrarCarrito(TpCompra carrito){
	...
	if(carrito==NULL){
		...
		cout<<" +-------------------------------------------+"<<endl;
		cout<<" | "<<left<<setw(13)<<"CODIGO"<<"| "<<left<<setw(10)<<"CANTIDAD"<<"| "<<left<<setw(15)<<"IMPORTE"<<"| "<<endl;
		cout<<" +-------------------------------------------+"<<endl;
		while(p!=NULL){
		...
		cout<<" | "<<setw(25)<<"COSTO TOTAL:"<<"| "<<left<<setw(15)<<total_pedido<<"|"<<endl;
		cout<<" +-------------------------------------------+"<<endl<<endl;
	}
	return total_pedido;
}
...
TpProducto consultarProducto(TpProducto lista, int codigo){			//Retorna el producto buscado por codigo
	TpProducto p = lista;
	while(p!=NULL){
		if (p->codigo==codigo){
			return p;
...
void comprar(TpProducto &catalogo, TpPedido &cola_pedidos, TpCliente &cliente, int &id){
	...
	do{
		system("CLS");
		cout<<"\t\tBIENVENIDO '"<<cliente->nombre<<"'"<<endl<<endl;
		mostrarCatalogo(catalogotemp);// catalogotemp 
		cout<<endl<<"Tu Carrito:"<<endl;
		...
		do {
			cout<<" Desea comprar mas? [S/N]: ",cin>>mas;
			mas=toupper(mas);
		} while (mas != 'S'  && mas != 'N' );
...
void reabastecer_cancelado(TpCompra carrito, TpProducto &lista){		//Reabastecer productos si se cancela pedido
...
int main(){
	...
	do{
		...
		switch(opc1){
			case 1:{
				if(usuarios==NULL){
					cout<<endl<<"\tERROR: NO HAY USUARIOS REGISTRADOS"<<endl<<endl;
					system("pause");
				}
				else{
					...
					if(cliente->nombre!=""){
						...
						if(cliente->contraseña==pass){
							cout<<endl<<"\tAUTENTICACION EXITOSA! BIENVENIDO, "<<cliente->nombre<<endl<<endl;
							system("pause");
							do{
								system("cls");
								menu2();
							...
							cout<<endl<<"\tERROR: NOMBRE DE USUARIO Y CONTRASENA NO COINCIDEN"<<endl<<endl;
							system("pause");
						}
					}
					else system("pause");
			...
			case 2:{
				...
				cout<<"\tUSUARIO REGISTRADO EXITOSAMENTE!"<<endl<<endl;
				system("pause");
				break;
			}
		}
